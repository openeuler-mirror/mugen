...
#@Desc      	:   Take the test mount on block
#####################################
...
function run_test() {
    ...
    mount /dev/$vggroup/$lv1 $blk 2>&1 | grep "mount point is not a directory"
    CHECK_RESULT $? 0 0 "mount ext3 on character block unexpectly."
    mount /dev/$vggroup/$lv2 $blk 2>&1 | grep "mount point is not a directory"
    CHECK_RESULT $? 0 0 "mount ext4 on character block unexpectly."
    mount /dev/$vggroup/$lv3 $blk 2>&1 | grep "mount point is not a directory"
